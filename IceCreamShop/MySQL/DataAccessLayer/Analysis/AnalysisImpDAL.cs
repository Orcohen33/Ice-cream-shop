...
namespace IceCreamShop.MySQL.DataAccessLayer.Analysis
{
    internal class AnalysisImpDAL
    {
        ...
        public string customerInvoice(int sid)
        {
            ...
            {
                dbContext.conn.Open();
                string sql = "SELECT * FROM `icecreamstore`.`sales` WHERE sid = " + sid;
        ...
        public string endOfDayReport(string date)
        {
            ...
            {
                dbContext.conn.Open();
                string sql = "SELECT order_date ,count(sid) as total_sales, sum(price) as total_price, avg(price) as average_price FROM `icecreamstore`.`sales` WHERE order_date = '" + date + "';";
        ...
        public string allEndOfDayReports()
        {
            ...
            {
                dbContext.conn.Open();
                string sql = "SELECT order_date ,count(sid) as total_sales, sum(price) as total_price, avg(price) as average_price FROM `icecreamstore`.`sales` group by order_date;";
                MySqlDataReader reader = new MySqlCommand(sql, dbContext.conn).ExecuteReader();
                while (reader.Read())
                {
        ...
        public string showAllIncompleteSales()
        {
            ...
            dbContext.conn.Open();
            string sql = "SELECT * FROM `icecreamstore`.`sales` WHERE price IS NULL";
            MySqlDataReader reader = new MySqlCommand(sql, dbContext.conn).ExecuteReader();
            while (reader.Read())
            {
        ...
        public string mostCommonIngredientNTaste()
        {
            ...
            string sql = "SELECT ingredients.name as name, ingredients.type as type, count(orders.iid) as count " +
                         "FROM `icecreamstore`.`orders` JOIN `icecreamstore`.`ingredients` " +
